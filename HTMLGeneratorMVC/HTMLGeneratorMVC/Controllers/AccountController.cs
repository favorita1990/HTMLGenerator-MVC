...
namespace HTMLGeneratorMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private RoleManager<Identity2Role, int> RoleManager;
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    userRoleName = (from r in userRoles
                                    from u in r.Users
                                    where u.UserId == user.Id
                                    select r.Name).FirstOrDefault();
                    if (userRoleName == "Admin")
                    {
                        message = string.Format("Welcome Mr. Admin {0}!", user.FirstName);
                        return RedirectToAction("Index", "Home", new { area = "Admin", message = message });
                    }
                    else
                    {
                        if (user.Genter == "1")
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string genter)
        {
            ...
            string message = null;
            RoleManager = new RoleManager<Identity2Role, int>(new RoleStore(new GeneratorDb()));
            if (ModelState.IsValid)
            {
